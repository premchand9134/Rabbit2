...
const FilterSidebar = () => {
    ...
    const [prizeRange, setPriceRange] = useState([0,100])

    const categories = ["Top Wear", "Bottom Wear"]

    const colors = ["Red", "Blue", "Green", "Black", "White","Yellow","Pink","Gray","Beige","Navy"]

    const sizes = ["XS","S", "M", "L", "XL", "XXL"]

    const materials = ["Cotton", "Polyester", "Wool", "Silk", "Linen"," Denim","Viscose","Fleece"]
    const brands = ["Urban Threads", "Modern Fit", "Street Styles", "Trendy Tees", "Beach Breeze","Fashionista","ChicStyle"]
    ...
    <div className='p-4'>
        ...
        <div className="mb-6">
            ...
            <div className=" flex flex-wrap gap-2">
                {colors.map((color)=>{
                    return(
                       <button  key={color} name='color' value={color} onClick={handleFilterChange} className={` w-8 h-8 rounded-full border border-gray-300 cursor-pointer transition hover:scale-105 ${filter.color === color ? "ring-2 ring-blue-500" : ""}`} style={{backgroundColor:color.toLocaleLowerCase()}}></button> 
                    )
                })}
            </div>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className=" mb-8">
            ...
            <input type="range" name='priceRange' min={0} max={100} value={prizeRange[1]} onChange={handlePriceChange} className=' w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer' />
